...
df_q1 <- 
  tibble(
    x = runif(n), # default max/min is 0, 1
    y = runif(n)
  )# Generate the data
...
df_q2 <- df_q1 %>%
  mutate(
    point_in_circle = (x^2 + y^2) < 1
  ) %>%
  summarize(
    point_in_circle_mean = mean(point_in_circle),
    pi_est =  point_in_circle_mean * 4,
    point_in_circle_sd = sd(point_in_circle),
    point_in_circle_se = point_in_circle_sd / sqrt(n)
  )
...
```
    ...
    ## # A tibble: 1 × 4
    ##   point_in_circle_mean pi_est point_in_circle_sd point_in_circle_se
    ##                  <dbl>  <dbl>              <dbl>              <dbl>
    ## 1                 0.79   3.16              0.407            0.00407
...
lo_pi <- (df_q2$point_in_circle_mean - q95 * df_q2$point_in_circle_se) * 4
hi_pi <- (df_q2$point_in_circle_mean + q95 * df_q2$point_in_circle_se) * 4
...
- What confidence level did you choose?
  - I chose a 95% confidence interval
- Was your sample size $n$ large enough? Why do you say that?
  - The sample size n that I chose was 10,000 points. We are only trying
    to get a reasonable estimate of pi in order to demonstrate the Monte
    Carlo theorem, so it does not need to be extremely precise, and the
    sample size was large enough that a 95% confidence interval did
    include the true value of pi, so the sample size was large enough
    for our purposes.
