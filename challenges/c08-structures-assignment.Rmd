...
- To what extent can you tell what shape the distribution of the data has?
  - It is hard to tell the shape of the distribution of the data, but it appears to be centered around 40000 psi. 
- Assuming the scopus is the strength of an individual part made from this aluminum alloy, is the observed variability real or induced?
  - The observed variability is real since we are considering this the true breaking strength of each individual sample.
...
- Is this estimate of the probability of failure trustworthy? Why or why not?
  - No. There are only 25 samples and there is wide variability in the data, and we can't determine the distribution from just these samples. We would need more samples to make the estimate trustworthy.
- Can you confidently conclude that `POF < 0.03`? Why or why not.
  - No, since the estimate of probability of failure is not trustworthy because of the variability in strength and the small sample size, we cannot confidently conclude that the point of failure is less than 0.03.
...
- Assuming your scopus is the probability of failure `POF` defined above, does your estimate exhibit real variability, induced variability, or both?
  - The estimate exhibits both induced variability due to the Monte Carlo sampling, and the real variability because the original samples themselves exhibited real variability.
- Does this confidence interval imply that `POF < 0.03`?
  - Yes, the confidence interval implies that POF is less than 0.03 because the entire confidence interval is below 0.03.
- Compare this probability with your estimate from q2; is it more or less trustworthy?
  - Compared to the estimate from q2 it is more trustworthy because there are many more samples and the confidence interval has an estimate of uncertainty associated with it, and they still lie below 0.03.
- Does the confidence interval above account for uncertainty arising from the *Monte Carlo approximation*? Why or why not?
  - Yes, because the mean and standard deviation and therefore the confidence interval is generated from the Monte Carlo samples.
- Does the confidence interval above account for uncertainty arising from *limited physical tests* (`df_samples`)? Why or why not?
  - No, because the mean and standard deviation and therefore the confidence interval is still dependent upon the original 25 samples.
- What could you do to tighten up the confidence interval?
  - Increase the number of samples
- Can you *confidently* conclude that `POF < 0.03`? Why or why not?
  - The confidence interval still does not account for the uncertainty arising from the real variability from the original samples or the limited number of samples.
...
- How does this estimate compare with your Monte Carlo estimate above?
  - The estimate is very close to the Monte Carlo estimate above but does not have an uncertainty estimate associated with it.
- Does this estimate have any uncertainty due to *Monte Carlo approximation*? Why or why not?
  - No because it is not using Monte Carlo approximation to generate the estimate
- With the scopus as the `POF`, would uncertainty due to *Monte Carlo approximation* be induced or real?
  - It would be induced because the Monte Carlo approximation is what would cause the uncertainty
- Does this estimate have any uncertainty due to *limited physical tests*? Why or why not?
  - Yes, the mean and standard deviation is still based on the 25 physical samples, so the estimate has uncertainty due to the limited physical tests.
- With the scopus as the `POF`, would uncertainty due to *limited physical tests* be induced or real?
  - The uncertainty due to limited physical tests is induced because it's a result of the limited number of tests, which can be changed, rather than a physical property of the material.
...
- Does the confidence interval above account for uncertainty arising from *limited physical tests* (`df_samples`)? Why or why not?
  - No, the confidence interval above does not account for uncertainty arising from limited physical tests because the uncertainty is still dependent on the number of samples which has not changed.
- Can you confidently conclude that `POF < 0.03`? Why or why not?
  - No, because the upper bound of the confidence interval is above 0.03, so part of the confidence interval lies above 0.03. 
