...
-   
    ...
    -   mu_f and d_p values are extremely small, on the order of approximately 10\^-5 magnitude

-   x, L, W, lam_f, rho_f, eps_p, T_norm values are relatively small, range from approximately 0.01 to 2

-   N_p, I_0, avg_q values are quite large, in the hundreds of thousands and greater

-   For most variables the minimum and maximum values are approximately in the same order of magnitude
...
-   Which model is *most accurate*? Which is *least accurate*?
    -   `fit_nonphysical` is the least accurate and `fit_cheat` is the most accurate
-   What *Category* of variable is `avg_T`? Why is it such an effective predictor?
    -   avg_T is an output, just like T_norm, and it is an effective predictor because it is an output of the same test that "output" the T_norm value.
-   Would we have access to `avg_T` if we were trying to predict a *new* value of `T_norm`? Is `avg_T` a valid predictor?
    -   No, we would not have access to avg_T, because again it is an output of the same test that output the T_norm value. It is not a value that we would know before the test so it is not a valid predictor.
-   What *Category* of variable is `idx`? Does it have any physical meaning?
    -   idx is metadata and it does not have any physical meaning. It is just a value to keep track of each simulation run.
...
-   k_f (Turbulence level) and N_p (number of particles) don't seem useful or seem to affect the rsquared value
-   Adding W, mu_f, and rest of the predictors decreases the r squared value on validate
...
-   Which model tends to be more accurate? How can you tell from this predicted-vs-actual plot?
    -   The residuals on the left model seem smaller, so the left model seems slightly more accurate.
-   Which model tends to be *more confident* in its predictions? Put differently, which model has *narrower prediction intervals*?
    -   The PI bars on the left model seem slightly smaller, so the left model seems slightly more more confident in its predictions, but it is difficult to tell
-   How many predictors does the `fit_simple` model need in order to make a prediction? What about your model `fit_q4`?
    -   fit_simple only uses one predictor, X, my model fit_q4 uses 4 predictors, x, T_f, L, and d_p
...
-   How much do you trust your model? Why?
    -   I would trust my model because the T_norm values for the simulation results fall within the predicted T_norm values for the vast majority of the validation set.
-   What kind of interval---confidence or prediction---would you use for this task, and why?
    -   I would use a prediction interval because we are trying to find predict a general range of values rather than a single value.
-   What fraction of validation cases lie within the interval you predict? How does this compare with `pr_level`?
    -   0.95 of the simulation results fall within the model which exceeds the expected level of 0.8.
-   What interval for `T_norm` would you recommend the design team to plan around?
    -  I would recommend the design team to plan around 1.279 to 2.568 for T_norm.
-   Are there any other recommendations you would provide?
    -   Fixing the values of T_f and d_p does not create a more accurate model so I would recommend that the team does not need to unnecessarily fix those values
