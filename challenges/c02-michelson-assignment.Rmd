...
- There are 46 measurements of "good" distinctness, 39 measurements of "fair" distinctness, and 15 measures of "poor" distinctness.
- The velocity is highest for measurements where the image distinctness is "good", and lowest for measurements where the image distinctness is "poor". The difference in velocity is approximately 60 km/s.
- The velocity values in the re-created table are approximately 1-2 km/s off the original table. 
-  It's possible that Michelson only had five  significant figures and had to round his data to the nearest tens, which could account for the difference in my table and the original table.
...
- Michelson's estimate of the error is 51 km/s, which is approximately one-third of the actual error, which is 151.542 km/s.
...
- Both graphs have several measurements that are outside the control limits.
- Both graphs demonstrate fluctuation around the control line over time.

Differences

- The "mean" line in the real graph jumps outside of the control limits approximately four times, compared to the simulated graph where the "mean" line only jumps outside of the control limits twice.
- Qualitatively, the "mean" line seems to fluctuate slightly less in the simulated data than in the real data.
- In the real graph, the velocity in vacuum measurements that are outside of the control limits were taken towards the beginning, whereas the velocity in vacuum measurements that are outside of the control limits in the simulated graph were taken towards the end.
...
- Looking at the graph of `VelocityVacuum` vs. `Temp`, there is a generally positive association between higher temperature and higher `VelocityVacuum` calculation. However, there are many outliers, and it visually does not appear to be a strong correlation. One could conclude that measurements taken at higher temperature result in a larger difference between Michelson's estimate and `LIGHTSPEED_VACUUM`.
- In general, it appears that higher distinctiveness images result in a higher `VelocityVacuum` value.
