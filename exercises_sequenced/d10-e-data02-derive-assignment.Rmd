...
version one accepts a dataframe as its first argument for `filter`. version two uses a pipe so that the diamonds dataframe is piped into `filter`.
...
When you uncomment `group_by`, the result changes from a price summary of the entire dataframe to a price summary of subsets of the dataframe, grouped by color and clarity.
...
- `cov_depth` is approximately 0.023 (unitless) while `cov_diff` is 498 (unitless), `cov_diff` is many orders of magnitude greater than `cov_depth`. 
- Why is `cov_diff` so large? This shows that there is a big variation in the diamonds dataset in the differences between actual depth and calculated depth. Some of the diamonds have a stated depth that is quite close to the calculated depth, while other diamonds have a stated depth that is very different from the calculated depth.
...
df_q5 <-
  diamonds %>%
    group_by(cut) %>%
    summarise(price_mean = mean(price, na.rm = TRUE), price_sd = sd(price, na.rm = TRUE), price_cov = price_sd/price_mean)
...
- There is the greatest amount of variance in price for ideal, variance increases from least to most ideal cut.
