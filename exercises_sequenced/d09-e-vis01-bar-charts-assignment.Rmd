...
**Observations**:
- In the prior graph (without changes), it seems like it is adding together all of the values of `hwy` for the given value of `cty`, which is not an accurate representation of the data.
...
**Observations**:
- The v1 visualization displays the number of instances of each class and varies the color by the class. Color is mostly aesthetic and does not add any additional information to the graph.
...
**Observations**:
- The v2 visualization displays the count of each class and varies the color by the type of drive (`drv` variable). Color thus demonstrates the count, within each class, depending on `drv`.

**Compare `v1` and `v2`**:

- Which visualization---`v1` or `v2`---enabled you to make more observations?

v2 allows us to make more observations because the added color allows you to deduce the count based on `drv` *within* each class,

- What was the difference between `v1` and `v2`?
In v1, the fill is redundant because each bar is already being separated by color. v2, the fill is not redundant because the color indicates the `drv` value.
...
**Observations**
- audi has a lot of compact cars.
- jeep, land rover, lincoln, and mercury only sell SUVs according to this dataset.
- 
