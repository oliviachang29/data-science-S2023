...
- The mean departure delay time is about 12.64 minutes
...
- F9, EV, YV, FL, and WN are at the top of the mean departure delay. FL, WN, F9, UA, and VX are at the top of the median departure delay.
- Which carriers differ in the two lists?
- EV and YV is in the mean but not median. UA and VX is at the top of the median but not mean.
- What do negative values of `dep_delay` mean?
- Negative values of departure delay mean that the flight departed early.
...
test_iqr <- quantile(v_test_iqr, probs = c(0.75), na.rm = TRUE) - quantile(v_test_iqr, probs = c(0.25), na.rm = TRUE)
test_iqr
```
...
- Which carriers are at the top of both `sd` and `IQR` departure delay?
  - sd: HA, F9, FL, YV, EV; IQR: EV, YV, 9E, F9, FL.
- Which carriers differ in the two lists?
  - In sd but not IQR: HA. In IQR but not sd: 9E.
- Which carrier has the biggest difference between its `sd` and `IQR`? What might this mean?
  - HA has a large SD but small IQR. This might mean that there are large outliers in the dataset that pull the standard deviation larger and are outside the IQR.
...
```{r ex-cor-line}
slope <- -0.3 # Play with this value; observe the correlation
df_line <-
...
- When the slope is positive, the correlation is positive.
- For what values of `slope` is the correlation negative?
- When the slope is negative, the correlation is negative.
...
df_quad %>%
  summarize(r = cor(x, y, method = "pearson"), pho = cor(x, y, method = "spearman"))
...
- I would predict the correlations to be 1 because there is a perfect relationship between x and y.
- What are the actual correlations?
- The actual correlations are close to 0 for both the pearson and spearman method.
