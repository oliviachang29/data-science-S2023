...
df_q1 <-
  genes %>%
    unite(col = vals, parent1, parent2, remove = FALSE, sep = "") %>%
    pivot_wider(names_from = parent2, values_from = vals)
...
df_q2 <-
  alloys_mod %>%
    pivot_longer(cols = c("E00", "mu00", "E45", "mu45"), names_to = "variable_angle", values_to = "value") %>%
    separate(col="variable_angle", into=c("variable", "angle"), sep=-2) %>%
