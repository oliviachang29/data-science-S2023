...
df_q1 <-
  df_michelson %>%
    mutate(epsilon_c = c_true - c_meas)
...
df_q2 <-
  df_q1 %>%
    summarise(epsilon_mean = abs(mean(epsilon_c)), epsilon_sd = sd(epsilon_c))
...
The error mean is about twice the size of its standard deviation, so it is not that large
- Is `epsilon_mean` large or small compared to `c_michelson_uncertainty?`?
The mean is three times larger than c_michelson_uncertainty
...
# one way to do it that doesn't work with the check
# se = df_q2$epsilon_sd / sqrt(df_michelson %>% count)
# c_lo <- df_q2$epsilon_mean - q * se
# c_hi <- df_q2$epsilon_mean + q * se
# 
# df_q3 <- df_q2$epsilon_mean - q * se
# c_hi <- df_q2$epsilon_mean + q * se
...
df_q3 <-
  df_q1 %>%
    summarise(
      ...
      c_meas_sd = sd(c_meas),
      c_meas_n = n(),
      c_meas_se = c_meas_sd / sqrt(c_meas_n),
      c_lo = c_meas_mean - q * se,
      c_hi = c_meas_mean + q * se
    )
