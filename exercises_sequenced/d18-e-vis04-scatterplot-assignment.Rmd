...
- The gam trend looks generally linear for diamonds of carat between 0 and 2 and then linear again between carat 4 and 5.
...
mpg %>%
  ...
  geom_label_repel(data = df_annotate, mapping = aes(label = displ))
```
...
- Mileage is greatest with the lowest engine displacement values and generally decreases as the engine displacement increases, but then increases again for both cty and hwy mileage for engine displacements greater than 5 liters.
